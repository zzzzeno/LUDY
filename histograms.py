...
class HistogramsError(Exception):
    pass
...
class Bin(object):
    ...
    def __setattr__(self, name, value):
        if name=='boundaries':
            if not isinstance(value, tuple):
                raise HistogramsError("Argument '%s' for bin property "+\
                    ...
                    if isinstance(coordinate, tuple):
                        for dim in coordinate:
                            if not isinstance(dim, float):
                                raise HistogramsError("Coordinate '%s' of the bin"+\
                                  " boundary '%s' must be a float."%str(dim,value))
                    elif not isinstance(coordinate, float):
                        raise HistogramsError("Element '%s' of the bin boundaries"+\
                                          " specified must be a float."%str(bound))
        elif name=='wgts':
            if not isinstance(value, dict):
                raise HistogramsError("Argument '%s' for bin uncertainty "+\
                                          "'wgts' must be a dictionary."%str(value))
            for val in value.values():
                if not isinstance(val,float):
                    raise HistogramsError("The bin weight value '%s' is not a "+\
                                                                 "float."%str(val))   
    ...
    def get_weight(self, key='central'):
        ...
        try:
            ...
            raise HistogramsError("Weight with ID '%s' is not defined for"+\
                                                            " this bin"%str(key))
    ...
    def addEvent(self, weights = 1.0):
        ...
        for key in weights:
            ...
            try:
                ...
                raise HistogramsError('The event added defines the weight '+
    ...
    def combine(cls, binA, binB, func):
        ...
        if binA.boundaries != binB.boundaries:
            raise HistogramsError('The two bins to combine have'+\
        ...
        try:
            ...
            raise HistogramsError("When combining two bins, the provided"+\
    ...
    def __init__(self, list = [], bin_range = None, 
        ...
        if bin_range:
            ...
            if len(bin_range)!=3 or any(not isinstance(f, float) for f in bin_range):
                raise HistogramsError("The range argument to build a BinList"+\
...
class Histogram(object):
    ...
    def __setattr__(self, name, value):
        if name=='title':
            if not isinstance(value, str):
                raise HistogramsError("Argument '%s' for the histogram property "+\
                                          "'title' must be a string."%str(value))
        elif name=='dimension':
            if not isinstance(value, int):
                raise HistogramsError("Argument '%s' for histogram property "+\
                                    "'dimension' must be an integer."%str(value))
            if self.allowed_dimensions and value not in self.allowed_dimensions:
                raise HistogramsError("%i-Dimensional histograms not supported "\
            ...
            if not isinstance(value, BinList):
                ...
                for bin in value:
                    if not isinstance(bin, Bin):
                        raise HistogramsError("Element '%s' of the "%str(bin)+\
                                  " histogram bin list specified must be a bin.")
        elif name=='type':
            if not (value is None or value in self.allowed_types or 
                                                        self.allowed_types==[]):
                raise HistogramsError("Argument '%s' for histogram"%str(value)+\
            ...
            if not value in self.allowed_axis_modes:
                raise HistogramsError("Attribute '%s' of the histogram"%str(name)+\
    ...
    def combine(cls, histoA, histoB, func):
        ...
        if len(histoA.bins)!=len(histoB.bins):
            raise HistogramsError('The two histograms to combine have a '+\
...
class HwU(Histogram):
    ...
    class ParseError(HistogramsError):
    ...
    def __init__(self, file_path=None, weight_header=None,
        ...
        if file_path is None:
            ...
            raise HistogramsError("Argument file_path '%s' for HwU init"\
    ...
    def output(self, path=None, format='HwU', print_header=True):
        ...
        if not format in HwU.output_formats_implemented:
            raise HistogramsError("The specified output format '%s'"%format+\
    ...
    def process_histogram_name(self, histogram_name):
        ...
        for i, tag in enumerate(histogram_name.split('|')):
            if i==0:
                ...
                if len(stag)!=2:
                    raise HistogramsError('Specifier in title must have the'+\
            " syntax @<attribute_name>:<attribute_value>, not '%s'."%tag.strip())
                ...
                if stag[0] in ['T','TYPE']:
                    ...
                    raise HistogramsError("Specifier '%s' not recognized."%stag[0])                    
    ...
    def set_uncertainty(self, type='all_scale',lhapdfconfig='lhapdf-config'):
        ...
        if type.upper()=='MUR':
            ...
            raise HistogramsError(' The function set_uncertainty can'+\
    ...
    def select_central_weight(self, selected_label):
        ...
        if selected_label not in self.bins.weight_labels:
            raise HistogramsError("Selected weight label '%s' could not be found in this HwU."%selected_label)
    ...
    def rebin(self, n_rebin):
        ...
        if n_rebin < 1 or not isinstance(n_rebin, int):
            raise HistogramsError("The argument 'n_rebin' of the HwU function"+\
    ...
    def get_x_optimal_range(cls, histo_list, weight_labels=None):
        ...
        if len(all_boundaries)==0:
            ...
            if len(all_boundaries)==0:
                raise HistogramsError("The histograms with title '%s'"\
    ...
    def get_y_optimal_range(cls,histo_list, labels=None, 
        ...
        # Negative weights are shown with dashed curve and plotted posiive, so we must apply an absolute value here.
        all_weights = sorted([abs(wgt) for wgt in all_weights if scale!='LOG' or wgt!=0.])
        if len(all_weights)!=0:
            partial_max = all_weights[int(len(all_weights)*0.95)]
            partial_min = all_weights[int(len(all_weights)*0.05)]
            overall_max         = all_weights[-1]
            overall_min         = all_weights[0]
        else:
            if scale!='LOG':
                return (0.0,1.0)
            else:
        ...
        if (overall_max-partial_max)>2.0*(partial_max-partial_min):
            y_max = partial_max
        else:
            y_max = overall_max
        
        # If the maximum is too far from the 90% max, then take the partial max
        if (partial_min - overall_min)>2.0*(partial_max-partial_min) and overall_min != 0.0:
            y_min = partial_min
        else:
            y_min = overall_min
        ...
        try:
            ...
            if not merging_scale is None: 
                ...
                if selected_label is None:
                    raise HistogramsError("No weight could be found in the input HwU "+\
...
#                                                  type_order.index(histo.type)))
    ...
    def parse_histos_from_PY8_XML_stream(self, stream, run_id=None, 
        ...
        if selected_run_node is None:
            if run_id is None:
                raise HistogramsError('No histogram was found in the specified XML source.')
            else:
                raise HistogramsError("Histogram with run_id '%d' was not found in the "%run_id+\
        ...
        for wgt_position, wgt_label in \
            ...
            for wgt_item in wgt_label.strip().split('_'):
                ...
                if len(property) == 2:
                    ...
                    raise HistogramsError("The weight label property %s could not be parsed."%wgt_item)
        ...
        if 'xmin' not in all_weights[0] or \
           ...
           'WeightError' not in all_weights[3]:
            raise HistogramsError('The first weight entries in the XML HwU '+\
...
# ===========  END HELPER FUNCTIONS ===========
                            ...
                            for wgt_label in selected_weights[j]:
                                if wgt_label == 'boundary_xmin':
                                    ...
                                    if weight.group('weight').upper()=='NAN':
                                        raise HistogramsError("Some weights are found to be 'NAN' in histogram with name '%s'"%hist_name+\
    " and jet sample multiplicity %d."%multiplicity)
                    ...
                    if len(bin_weights)!=len(ordered_weight_label_list):
                        raise HistogramsError('Not all defined weights were found in the XML source.\n'+\
...
#                          stop
    ...
    def output(self, path, format='gnuplot',number_of_ratios = -1, 
        ...
        if len(self)==0:
            return HistogramsError, 'No histograms stored in the list yet.'
        ...
        if isinstance(path, str) and not any(ext in os.path.basename(path) \
            ...
            raise HistogramsError("The path argument of the output function of"+\
...
#                                      'was divided by a bin with zero weight.')
        ...
        for histo in self[1:]:
            if (not mu_var_pos is None) and \
               ...
               raise HistogramsError('Not all histograms in this group specify'+\
...
plot \\"""
        ...
        # No longer do that as we now automatically have gnuplot render negative weights as positive with dashed line pattern.
